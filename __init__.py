...
from ecowitt import Websocket
from ecowitt.errors import WebsocketError
from ecowitt.util import get_public_device_id
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Set up the Ecowitt as config entry."""
    if not entry.unique_id:
        hass.config_entries.async_update_entry(
            entry, unique_id=entry.data[CONF_APP_KEY]
        )

    ecowitt = EcowittStation(
        hass,
        entry,
    ...
    try:
        await ecowitt.ws_connect()
    except WebsocketError as err:
        LOGGER.error("Config entry failed: %s", err)
        raise ConfigEntryNotReady from err

    hass.data.setdefault(DOMAIN, {})[entry.entry_id] = ecowitt

    async def _async_disconnect_websocket(_: Event) -> None:
        await ecowitt.websocket.disconnect()
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Unload an Ecowitt config entry."""
    unload_ok = await hass.config_entries.async_unload_platforms(entry, PLATFORMS)
    if unload_ok:
        ecowitt = hass.data[DOMAIN].pop(entry.entry_id)
        hass.async_create_task(ecowitt.ws_disconnect())
...
class EcowittStation:
    """Define a class to handle the Ecowitt websocket."""
    ...
    async def ws_connect(self) -> None:
        ...
        def on_data(data: dict) -> None:
            ...
            async_dispatcher_send(self._hass, f"ecowitt_station_data_update_{mac}")
...
class EcowittWeatherEntity(Entity):
    """Define a base Ecowitt entity."""

    _attr_should_poll = False

    def __init__(
        self,
        ambient: EcowittStation,
        mac_address: str,
        station_name: str,
    ...
    async def async_added_to_hass(self) -> None:
        ...
        def update() -> None:
            ...
            if self.entity_description.key == TYPE_SOLARRADIATION_LX:
                ...
                self._attr_available = (
                    self._ecowitt.stations[self._mac_address][ATTR_LAST_DATA][
        ...
        self.async_on_remove(
            async_dispatcher_connect(
                self.hass, f"ecowitt_station_data_update_{self._mac_address}", update
            )
        )
